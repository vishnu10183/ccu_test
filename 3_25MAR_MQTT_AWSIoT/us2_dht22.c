...
float dist_cm[30] , humid =0.0, tempC = 0 ;
...
void *ultraS( void *param ){
	...
	{
		FILE *fp;
		if (DEBUG) fprintf( stdout,"Reading DHT File..." );
		fp = fopen( tempDataFile, "r" );
		fseek( fp, 0, SEEK_SET );
		fscanf( fp, "%f", &tempC );
		fscanf( fp, "%f", &humid );
		if (DEBUG){
	...
	if(DEBUG) fprintf( stdout, "Starting side HCSR04\n" );
